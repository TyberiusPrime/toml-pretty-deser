...
### Calling `sync_nested_state` in custom `Visitor` implementations

`sync_nested_state()` propagates error states from children upward through the
container hierarchy.  It is called **automatically** after every `verify()` run
for all types generated by `#[tpd]` — you never need to think about it in normal
usage.
