...
use toml_pretty_deser::{
    DeserError, TomlCollector, TomlHelper, TomlValue,  suggest_alternatives,
};
//library code
//
use toml_pretty_deser::helpers::{Root, VerifyVisitor, Visitor, deserialize_toml};
...
impl VerifyVisitor<Root> for PartialOuter {
    fn vv_validate(mut self, helper: &mut TomlHelper<'_>, _parent: &Root) -> Option<Self>
    where
        Self: Sized + Visitor,
    {
...
impl VerifyVisitor<PartialOuter> for PartialNestedStruct {
    ...
    fn vv_validate(mut self, helper: &mut TomlHelper<'_>, parent: &PartialOuter) -> Option<Self> {
