...
impl VerifyIn<TPDRoot> for PartialBarcodeConfig {
        ...
        Self: Sized + toml_pretty_deser::Visitor,
    {
        ...
        if let Some(opt_map) = self.barcodes.value.as_mut()
            && let Some(map_and_keys) = opt_map.as_mut()
        {
...
";
    ...
    assert!(result.is_err(), "verify marks all keys invalid");
    if let Err(e) = result {
        assert!(
            e.pretty("test.toml")
                .contains("Hint: All keys are forbidden in this test")
        );
...
"#;
    let result: Result<_, _> =
        BarcodeConfig::tpd_from_toml(toml, FieldMatchMode::Exact, VecMode::Strict);
    assert!(result.is_err(), "foo has wrong type so parsing fails");
    if let Err(e) = result {
