...
fn test_deser_error_std_error_impl() {
    ...
    let result: Result<Outer, _> =
        Outer::tpd_from_toml("this is not valid toml ===", FieldMatchMode::Exact, VecMode::Strict);
    assert!(result.is_err());
    if let Err(e) = result {
        assert!(
    ...
    let result: Result<Outer, _> =
        Outer::tpd_from_toml("# all required fields missing", FieldMatchMode::Exact, VecMode::Strict);
    assert!(result.is_err());
    if let Err(e) = result {
        assert!(
